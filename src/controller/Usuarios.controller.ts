...
export class UsuariosController {
  ...
  public deletar = async (req: Request, res: Response) => {
    try {
      ...
      if (!result)
        res.status(400).json({ message: "Nao foi possivel deletar" });
      res.status(200).json(result);
    } catch (err) {
  ...
  public atualizar = async (req: Request, res: Response) => {
    try {
      const result = await this.usuariosServs.atualizarUser({
        id: req.params.id,
        ...req.body,
      });
      res.status(200).json(result);
      ...
      res
        .status(400)
        .json({ message: "Nao foi possuvel atualizar este usuario" });
    }
  };
}
